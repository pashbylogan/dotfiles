...
| Desktop (Linux) | `sway`, `swaylock`, `swayidle`, `kanshi`, `i3status`, `rofi`, `grim`, `slurp`, `wl-clipboard`, `brightnessctl`, `mako`, `thunar`, `pavucontrol`  |
...
`zsh`, `tmux`, `nvim`, `ssh`, `ghostty`, `sway`, `i3status`, `mako`, `kanshi`, plus user scripts in `bin`.
