...
Prompt replaced with Starship (Rust binary, low single-digit ms render). Installed to `~/apps/bin` via curl installer.

### 12. Replaced nvm with fnm

nvm added ~300ms to every shell open (even with lazy-loading, the first `node`/`npm` call paid the full penalty). fnm is a Rust binary an order of magnitude faster (~5–15 ms shell init in published benchmarks vs. nvm's ~75–500 ms), reads `.nvmrc` natively, and is a near-drop-in replacement. Installed to `~/apps/fnm` via curl installer.
...
The config deliberately leans on upstream defaults for layout (heights, margins, module formats are unset). A small `style.css` *is* stowed — added later in commit `5ec3863` ("style: add i3-style Waybar status") to set a flat black i3-style background, the Cascadia Mono Nerd Font stack, and a minimal palette. The earlier intent (no stylesheet, rely on `/etc/xdg/waybar/style.css`) didn't survive once we needed the bar to match the rest of the desktop chrome. Cosmetic drift is now ours, but kept to ~50 lines.
...
### 33. Event-driven runtime services and Ansible run-time wins

Two runtime helpers move from polling to event-driven for *responsiveness* (not measurable battery savings — a follow-up review found those numbers didn't survive scrutiny), plus a real Ansible-side speedup and a small zsh cleanup.

**`battery-monitor`: timer → UPower D-Bus listener.** Previously a `.timer` fired every 30 s and the `.service` re-parsed `upower -i` even when nothing had changed. The unit is now `Type=simple` and long-running; it subscribes to `org.freedesktop.UPower` via `gdbus monitor --system` and runs the discharge check only on `*/devices/battery_*` *and* `*/devices/line_power_*` `PropertiesChanged` events — both paths matched so AC unplug fires immediately instead of waiting up to 30 s for the next polled tick. The device path is resolved once at startup and reused. `Restart=on-failure` plus `StartLimitIntervalSec=60` / `StartLimitBurst=3` cap restart bursts on real failures while letting the script exit 0 cleanly on a no-battery host. Ansible disables the obsolete `.timer` on re-applies so the migration is idempotent. The win is responsiveness on AC events, not power: the absolute wakeup count went from ~120/hour to whatever cadence UPower itself signals at (single-digit hertz at most), well below the >300/sec threshold powertop flags as power-relevant. `lib/.local/lib/dotfiles/battery-monitor` is the script; the timer file is gone.

**`sway-idle-status`: 2 s poll → `swaymsg subscribe`.** The waybar `custom/idle` module was polled every 2 s, forking `swaymsg -t get_tree | jq` 30×/min for a state that changes rarely. Replaced with a continuous-output module: the script subscribes to `'["window"]'` events via `swaymsg -m -t subscribe`, recomputes idle/awake state on each event, and emits JSON only when the state actually changes (so a burst of focus or title-change events produces zero output if the awake/idle classification didn't flip). The `while read` loop uses process substitution (`< <(...)`) instead of a pipe so the de-dupe `last` variable persists across events in the main shell *and* `set -e` continues to trip on real script errors (waybar then respawns via `restart-interval: 1`). The win is that idle/awake classification updates the bar instantly when fullscreen toggles or an idle-inhibitor appears, rather than after up to 2 s of lag — there's no honest CPU/wakeup case to make here, since `window` events on an active session can outpace the old 0.5 Hz poll.

**Nautilus pre-warm — tried and reverted.** An earlier revision of this section pre-launched `nautilus --gapplication-service` from sway's `exec` block, claiming a permanent ~30 MB resident process for sub-50 ms file-manager opens. A follow-up review found this was wrong on both ends: `--gapplication-service` activates `G_APPLICATION_IS_SERVICE`, which by default exits within ~10 s when the use count drops to zero (Nautilus does not call `g_application_set_inactivity_timeout`), and the in-flight RSS while the service was alive was ~71 MB on Fedora 44, not 30. Net effect was a one-off ~71 MB session-start tax with no follow-on benefit unless the user opened Nautilus within ~10 s of login. Reverted; Nautilus cold-launches the normal way from rofi.

**Ansible run-time speedup (real win).** `ansible.cfg` gained `pipelining = True`, `gathering = smart`, and a 2 h `fact_caching = jsonfile`. Pipelining cuts SSH round-trips per task; smart+cached gathering skips fact gathering on subsequent runs within the cache window. Repeated `dot-apply` runs are noticeably faster.

**`.zshrc` cleanup.** Dropped the `fzf < 0.48` fallback path — Fedora 43+ ships current fzf, so the probe-and-source ladder was unreachable. Tiny simplification, not a measurable speedup.
